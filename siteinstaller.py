...
print('\033[1;91m [~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~]')
print(' [                                            ]')
print(' [        Dark_wave - Site Installer v' + flinstall_version + '      ]')
print('\033[94m [                                            ]')
print(' [~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~]')
print('\033[39m')
...
def check_Dark_wave():
	global installed_sites, dark_wave_cont
	try:
		fl = open('Dark_wave','rw')
		dark_wave_cont = fl.read()
		fl.close()
	except:
		print(language.COULD_NOT_OPEN_dark_wave)
		sys.exit()
	if(dark_wave_cont <= 100):
		print(language.NO_Dark_wave_FOUND)
		sys.exit()
	flv = ''
	flr = ''
	fld = ''
	if('version=' in dark_wave_cont) and ('revision=' in dark_wave_cont):
		flv = dark_wave_cont[dark_wave_cont.find('version=')+8:]
		flv = flv[:flv.find('\n')]
		flr = dark_wave_cont[dark_wave_cont.find('revision=')+9:]
		flr = flr[:flr.find('\n')]
	else:
		print(language.CORRUPTED_dark_wave)
		sys.exit()

	# Version check
	#vchk = False
	#for version in dark_wave_comp_versions:
	#	if(version == flv):
	#		vchk = True
	#if(vchk == False):
	#	print('Your Dark_wave version '+color.BOLD + flv  + color.END + '. \nSupported versions are - ' + ' - '.join(reversed(dark_wave_comp_versions)))		
	#	sys.exit()
	# ###############

	installed_sites = int(dark_wave_cont.count('elif [ "$webconf" ='))
	fls = str(installed_sites + 1)  

	# Check on double installation!
	fld = dark_wave_cont[dark_wave_cont.find('$DUMP_PATH/data/index.htm'):]
	if(site_name in fld):
...
def insert_at_secondlast_pos1():
	global dark_wave_cont, site_name, site_language
	whitespaces = ''
	...
	before = dark_wave_cont[:int(dark_wave_cont.rfind(search_string))]
	after = '\t\t\t' + dark_wave_cont[int(dark_wave_cont.rfind(search_string)):]
	dark_wave_cont = before + insert_site + after
# ##########################################

# ###### Second INSERT
def insert_at_secondlast_pos2():
	global installed_sites, dark_wave_cont, site_name
	site_number = str(installed_sites +1)
	insert_site = 'elif [ "$webconf" = "'+ site_number +'" ]; then\n\t\t\t\t' + site_name + '\n\t\t\t\tbreak\n\n\t\t\t'
	dark_wave_cont = dark_wave_cont[:int(dark_wave_cont.rfind('elif [ "$webconf" ='))] + insert_site + dark_wave_cont[int(dark_wave_cont.rfind('elif [ "$webconf" =')):]

def last_option_correct_number2():
	global installed_sites, dark_wave_cont
	acc = int(dark_wave_cont.rfind('elif [ "$webconf" ='))
	acc0 = int(dark_wave_cont[acc:].find('" = "')+5)
	before = dark_wave_cont[:acc+acc0]
	after = dark_wave_cont[acc+acc0+2:]
	dark_wave_cont = before + str(installed_sites+2) + after
# ##########################################

# ###### Third INSERT
def insert_at_last_pos3():
	global dark_wave_cont, site_name
	insert_site = '\n\nfunction ' + site_name + ' {\n\tmkdir $DUMP_PATH/data &>$dark_wave_output_device\n\tcp $WORK_DIR/Sites/' + site_name + '/* $DUMP_PATH/data\n\t}'
	before = dark_wave_cont[:int(dark_wave_cont.rfind('}'))+1]
	after = dark_wave_cont[int(dark_wave_cont.rfind('}'))+1:]
	dark_wave_cont = before + insert_site + after
# ##########################################
...
try:
	subprocess.Popen(['cp','Dark_wave', 'bckp_Dark_wave'])
except:
...
print('')
print(' # ' + language.RECONFIGURE_Dark_wave_BASH + '...')
try:
...
print('')
print(' # '+ language.REWRITING_Dark_wave_BASH + '...')
try:
	wdark_wave = open('Dark_wave','w')
	wdark_wave.write(dark_wave_cont)
	wdark_wave.close()
except:
	print(language.FATAL_ERROR + '[501]...')
	print(language.TRYING_TO_RESTORE_BACKUP)
	try:
		subprocess.Popen(['mv','bckp_Dark_wave', 'Dark_wave'])
		print('Dark_wave ' + language.BACKUP_RESTORED + '...')
	except:
...
try:
	dark_waveit = open('Dark_wave','r')
	integ_Dark_wave = dark_waveit.read()
	dark_waveit.close()
	if(len(integ_Dark_wave) == len(dark_wave_cont)):
		print(' # ' + language.DONE + '!')
		print(' # ' + language.DELETING_BACKUP + '...')
		subprocess.Popen(['rm','bckp_Dark_wave'])
	else:
